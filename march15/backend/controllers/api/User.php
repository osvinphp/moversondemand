    ...
    */
    ...
    public function bookmove_post(){
		...
		$distance_price=round($distance*$vehicle[0]->km_charges,2);
		$time_price=round(($time_duration/60)*$movers_charges,2);
	...
	public function transactionListing_post(){
        ...
        public function yourMoveList_post(){
            ...
            if($type==1){
              ...
              $acceptedbooking=$this->db->query("SELECT * from tbl_booking where user_id= '".$user_id."' and tbl_booking.is_accepted=0 and tbl_booking.is_started=0 and tbl_booking.is_completed=0 and tbl_booking.is_cancelled=0 ")->result();
    ...
    $this->User_model->updateNewpassword($message);
      ...
      public function moveAction_post(){
        ...
        $hypertrackUserId = $this->input->post('hypertrackUserId');
        $hypertrackActionId = $this->input->post('hypertrackActionId');
        $hypertracklookupid = $this->input->post('hypertracklookupid');
        ...
        $date=date('Y-m-d H:i:s');
        $time_duration=round((strtotime($date)-strtotime($moveDetail[0]->started_time)));

        // print_r($time_duration);die;
        $durationnew=round($time_duration/60,2);
        ...
        $time_price=round($durationnew*$movers_charges);

        $totalprice=(($distance_price+$time_price));
        ...
        if ($estimatesecond<=$date) {
          $bookingdata = $this->User_model->update_data('tbl_booking',array('driver_id'=>$driver_id,'is_started'=>1,'hypertrackActionId'=>$hypertrackActionId,'hypertracklookupid'=>$hypertracklookupid),array('id'=>$booking_id));
          $driverDetail = $this->User_model->update_data('tbl_driverDetail',array('hypertrackUserId'=>$hypertrackUserId),array('driver_id'=>$driver_id));
          $moveData = array(
          "driver_id"=>$driver_id,
          "status"=>2,
        ...
        /*for one time hit next hit get error*/
            ...
            return true;
              ...
              $estimated=$pricingDetail[0]->estimate_price;
              $abc=(($estimated/100)*$charge);

          /*extra price addition*/
          $price1= $totalprice+$extraprice;
          /*gst price calculate*/
          $gstprice=round(($pricingDetail[0]->gst_percentage*$price1/100),2);
          /*price with gst*/
          $grandtotal=round($price1+$gstprice,2);

          // print_r($totalPrice);
          // echo "gst+extra+total";
          ...
          if (!empty($bookingDetails[0]->promoid)) {
            ...
            $totalpercent=$grandtotal*($promopercentage/100);
        ...
        $amountdata=$grandtotal-$promomaxAmount;
          ...
          $amountdata=$grandtotal-$roundTotal;
            ...
            $amountdata=$grandtotal;
          }
          // print_r($amountdata);
          // echo "with promocode";

          $totalPrice=round($amountdata-$abc,2);
          // print_r($totalPrice);
          // echo "after booking";
          // die;
        ...
        if($trasactdata->success==1){
          /*if user balence more then deduct balence start*/
             ...
             $transResponse = $this->User_model->insert_data('tbl_transactions',$transArray);
            //  $pricedata = $this->User_model->update_data('tbl_movepricing',array('time_price'=>$time_price,'time'=>$time_duration,'extra_fare'=>$extraprice,'total_price'=>$totalPrice,'gst_price'=>$gstprice),array('move_id'=>$booking_id));
      ...
      // $getDriverWallet = $this->User_model->select_data('*','tbl_wallet',array('user_id'=>$bookingdriverDetails[0]->driver_id));
       ...
       $getAdminpercent=round($grandtotal*($bookingDetails[0]->admin_percentage/100),2);
       $Driverbalance=round($grandtotal-$getAdminpercent,2);
      ...
      /*for one time hit next hit get error*/
        ...
        if ($cancelUser_type==1) {   /*cancelled by driver start*/
          ...
          $txndetail=$this->db->query("SELECT * FROM tbl_transactions where user_id='".$bookingDetails[0]->user_id."' AND move_id='".$bookingDetails[0]->id."' ORDER BY date_created DESC Limit 1")->result();
            ...
            $amountdeduct = ($getbookingcharge / 100) * $pricingDetail[0]->estimate_price;
	...
	public function getcurrentMove_post(){
		...
		/*else give current date topmost from live server time start*/
			...
			$data['booking_data']=$this->db->query("SELECT * FROM tbl_booking where driver_id = '".$user_id."' AND ( is_accepted='1' OR is_started='1') AND is_completed='0' AND is_cancelled='0'  AND  booking_date = '".$date."' ORDER BY slot_starttime ASC LIMIT 1")->result();
			$acceptedbooking12=array();
			foreach ($data['booking_data'] as $key => $value) {
				$acceptedbooking12= unserialize($value->item_image);
				$acceptedbooking12=array_filter($acceptedbooking12);
	...
	//   	$data=$this->db->query("SELECT tbl_booking.*,tbl_movepricing.time,tbl_movepricing.estimate_price from tbl_booking join tbl_movepricing on  tbl_movepricing.move_id=tbl_booking.id where is_accepted=1 and is_started=0 and is_completed=0 and is_cancelled=0 ")->result();
 // 		print_r($data);
 // 		$array=array();
	// 	foreach ($data as $key => $value) {
	// 		$date=date("Y-m-d H:i:s");
	...
	// 	$data=$this->db->query("SELECT tbl_booking.*,tbl_movepricing.time,tbl_movepricing.time,tbl_movepricing.estimate_price from tbl_booking join tbl_movepricing on  tbl_movepricing.move_id=tbl_booking.id where is_accepted=1 and is_started=1 and is_completed=0 and is_cancelled=0 ")->result();
	// 	$date=date("Y-m-d H:i:s");
	// 	$array=array();
	// 	foreach ($data as $key => $value) {
	// 		$date=date("Y-m-d H:i:s");
	...
	// 			$array=$data;
	// 			// print_r($array);
	// 			$totalPrice=$array[0]->estimate_price;
	// 			$movecompletetime = $this->User_model->select_data('*','tbl_setting');
	// 			$charge= $movecompletetime[0]->min_booking_charge;
	// 			$estimated=$array[0]->estimate_price;
	// 			$abc=round(($estimated/100)*$charge,2);
  //
  //
  //
	// 			$gstprice=round(($pricingDetail[0]->gst_percentage*$totalPrice/100),2);
	// 			/*price with gst*/
	// 			$grandtotal=round($totalPrice+$gstprice-$abc,2);
  //
  //
	...
	// 					$roundTotal=($totalpercent);
	// 					if ($roundTotal>$promomaxAmount) {
	// 						$price=$grandtotal-$promomaxAmount;
	// 					}
	// 					else{
	// 						$price=$grandtotal-$roundTotal;
	// 					}
	// 					if ($promoDetail[0]->status=1) {
	...
	// 				);
	// 				$getDriverBalance = $this->User_model->select_data('*','tbl_driversFund',array('driver_id'=>$bookingdriverDetails[0]->id));
	// 				$getAdminpercent=round($grandtotal*($array[0]->admin_percentage/100),2);
	// 				$Driverbalance=round($grandtotal-$getAdminpercent,2);
	// 				$updatedriveramount=$getDriverBalance[0]->outstanding_amount+$Driverbalance;
	// 				$uptdriverDAta = $this->User_model->update_data('tbl_driversFund',array('outstanding_amount'=>$updatedriveramount,'date_modified'=>date('Y-m-d H:i:s')),array('driver_id'=>$bookingdriverDetails[0]->id));
  //
  //
	...
	// 				'admin_price'=>$getAdminpercent
	// 				);
	// 				$insertMove_history = $this->User_model->update_data('tbl_movepricing',$array1,array("move_id"=>$booking_id));
  //
	// 				$transUserArray = array(
	// 				'amount_credited'=>$Driverbalance,
	// 				'user_id'=>$driver_id,
  ...
  //   }
	...
	public function FindDriver_post(){
		$query  = $this->db->query("select * from tbl_booking where id = '".$_POST['book_id']."' ")->result();
		if($query[0]->is_accepted == 1){
			$result = array(
				"controller" => "User",
				"action" => "FindDriver",
				"ResponseCode" => true,
    ...
    public function getpendingMoves_post(){
        ...
        foreach ($pendingBooking as  $value) {
        	...
        	$getdata= $this->db->query("SELECT a.id,a.email,b.token_id,b.device_id,'".$value->id."' as bookingid,'".$value->destinationcity_id."' as destination,c.located_at FROM`tbl_users` AS a JOIN tbl_login AS b ON a.id = b.user_id JOIN tbl_driverDetail AS c ON (a.id = c.driver_id)   WHERE a.user_Type = 2 and b.status=1  AND a.id IN(SELECT driver_id from tbl_driverDetail where  vehicle_id='".$getvehicle[0]->id."' AND status=1 ) and a.id not in (select driver_id from tbl_declinedDriver where driver_id=a.id and move_id='".$value->id."')  AND a.id NOT IN ( SELECT driver_id  FROM `tbl_booking`  WHERE (is_accepted=1 or is_started=1) and is_completed=0 and is_cancelled=0  and (CONCAT('".$value->booking_date."', ' ', '".$value->slot_starttime."') BETWEEN CONCAT(booking_date, ' ', slot_starttime) AND DATE_ADD(CONCAT(booking_date, ' ', slot_starttime) , INTERVAL ('".round($getpricingDetail->time/60)."' + (('".$time."')*2)) MINUTE) OR DATE_ADD(CONCAT('".$value->booking_date."', ' ', '".$value->slot_starttime."'), INTERVAL('".round($getpricingDetail->time/60)."' + ('".$time."')*2) MINUTE) BETWEEN CONCAT(booking_date, ' ', slot_starttime) AND DATE_ADD(CONCAT(booking_date, ' ', slot_starttime) , INTERVAL ('".round($getpricingDetail->time/60)."' + (('".$time."')*2)) MINUTE)))")->result();
    ...
    public function upcomingSchdule_post(){
      	...
      	$pendingBooking=$this->db->query("SELECT tbl_booking.*,tbl_moveType.title,tbl_moveType.icon,tbl_users.fname,tbl_users.lname   from tbl_booking join tbl_users on tbl_users.id=tbl_booking.user_id join tbl_moveType on tbl_moveType.id=tbl_booking.moveType_id where   driver_id='".$driver_id."' and (tbl_booking.is_accepted=1 or tbl_booking.is_started=1) and  tbl_booking.is_completed=0 and tbl_booking.is_cancelled= 0  and tbl_booking.booking_date = '".$date."' ")->result();
        if ($pendingBooking){
            $result = array(
            "controller" => "User",
	...
	public function getHypertrasckDetail_post(){
		$bookid=$this->input->post('bookingid');
		$driver_id=$this->input->post('driver_id');
		$bookingprevDetails = $this->User_model->select_data('*','tbl_booking',array('driver_id'=>$driver_id,'is_accepted'=>1,'is_started'=>1,'is_completed'=>0,'is_cancelled'=>0));
		$bookingDetails = $this->User_model->select_data('*','tbl_booking',array('id'=>$bookid));
		...
		if (empty($bookingprevDetails)) {
			...
			if ($estimatesecond<=$date) {
				...
				"action" => "moveAction",
				"ResponseCode" => true,
				"MessageWhatHappen" => "You can start job"
				);
				print_r(json_encode($result));die;
			}
			else{
				...
				"action" => "moveAction",
				"ResponseCode" => false,
				"MessageWhatHappen" => "Ride cannot be start before time"
				);
				print_r(json_encode($result));die;
			}
		}
			...
			$result = array(
				...
				"action" => "moveAction",
				"ResponseCode" => false,
				"MessageWhatHappen" => "already started job"
			);
			print_r(json_encode($result));die;
		}
	}
