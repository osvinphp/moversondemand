                  ...
                  <div class="map-details">
                    ...
                    <div class="content-map-details Content_WraPper-inner move-details-inner">
                      ...
                      <div class="content-md-footer-details">
                        <div class="fd-detail fd-time alignment"><div class="fd-img"><p>Move #<?php echo $bookData->id; ?></p></div><p>
                          <?php
                              ...
                              $utcDateTime = date("Y-m-d", $asia_timestamp);
                              $bdate = date("d M y", strtotime($utcDateTime));

                              if($move_data[0]->status == 0){
                                $utcDateTime1 = $this->Admin_model->convertlocal($bookData->slot_starttime);
                                $startt =  date("h:i A.", strtotime($utcDateTime1));

                                $utcDateTime2 = $this->Admin_model->convertlocal($bookData->slot_endtime);
                                $endt = date("h:i A.", strtotime($utcDateTime2));
                                $ss = $startt."-".$endt;
                                echo $bdate.' '.$ss;
                                $text = 'Pending';
                              }elseif($move_data[0]->status == 1){
                                $tz = $move_data[0]->accepted_time;
                                $text = 'Assigned';
                              }
                              elseif($move_data[0]->status == 2){
                                $tz = $move_data[0]->started_time;
                                $text = 'EnRoute';
                              }
                              elseif($move_data[0]->status == 3){
                                $tz = $move_data[0]->completed_time;
                                $text = 'Completed';
                              }
                              else{
                                $tz = $move_data[0]->cancelled_time;
                                $text = 'Cancelled';
                              } 
                                if($move_data[0]->status == 1 || $move_data[0]->status == 2 || $move_data[0]->status == 3 || $move_data[0]->status == 4){
                                  $utcDateTime2 = $this->Admin_model->convertlocal($tz);
                      ...
                      <div class="content-md-footer-details center_Align">
                        <div class="fd-detail fd-time"><div class="fd-img"><img src="<?php echo base_url();?>public/images/move-details-time.png" class="img-responsive"></div><p><?php echo round(($pricing->time / 60) % 60)." mins"; ?></p></div>
...
<?php $moverDetails = $move_data[0]; if($moverDetails->status == 3 || $moverDetails->status == 1 || $moverDetails->status == 2){ ?>
        ...
        <?php 
            ...
            $cn_time = $this->Admin_model->convertlocal($bookData->slot_endtime);
            $budate = strtotime($utcDateTime.' '.$cn_time);
            // $sch_date = $st_date.' '.$budate;
