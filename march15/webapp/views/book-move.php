                                                    ...
                                                    // }else{
                                                        foreach ($defaultCard as $key => $value) {
                                                            if($value->is_default == 1){
                                                                if(empty($_SESSION['sel_card_name'])){
                                                                    $dnsa = 'XXXX XXXX XXXX '.$value->card_no;
                                                                }else{
                                                                    $dnsa = 'XXXX XXXX XXXX '.$_SESSION['sel_card_name'];
                                                                }
                                                            }else{
                                                                if(empty($_SESSION['sel_card_name'])){
                                                                    $dnsa = 'Add payment';
                                                         ...
                                                         echo '
                                                            <form action="/mycard" method="POST">
                                                                <button type="submit" name="cardsub"><i class="fa fa-credit-card" aria-hidden="true"></i>'.$dnsa.'<i class="fa fa-chevron-right floatt" aria-hidden="true"></i></button>
                                                            </form>';
                                                   // }
                                                ?>
                                            </b>
                                ...
                                <div class="previous">
                                   <!--  <a href="<?php echo base_url('moving'); ?>">
                                    ...
                                    <?php
                                        foreach ($defaultCard as $key => $value) {
                                            if($value->is_default == 1){
                                                $_SESSION['decardses'] = $value->id;
                                            }
                                        }
                                        $static_key = "hbdhwb!^^!&&!vghedwqe!@";
                                        $bacs = $_SESSION['decardses'].'_'.$static_key;
                                        $caiden = base64_encode($bacs);
                                        $bacs1 = $_SESSION['sel_card_id'].'_'.$static_key;
                                        $caiden1 = base64_encode($bacs1);
                                    ?>
                                    <?php  // if($defaultCard[0]->is_default == 1){ 

                                        if(isset($_SESSION['decardses'])){
                                    ?>
                                        <form class="NEXT_FORMBT" action="/find/<?php echo $caiden ; ?>" method="POST">       
                                            <button type="submit" name="submit" id="l">Next</button>
                                        </form>
                                    <?php } else { ?>
                                        <form class="NEXT_FORMBT" action="/find/<?php echo $caiden1; ?>" method="POST">  
                                            <button type="submit" name="submit" <?php if(isset($_SESSION['sel_card_id']) || isset($_SESSION['sel_card_name'])){ }else{echo "disabled='disabled'"; } ?>>Next</button>
                                        </form>
                                    <?php } ?>
                                 </div>
                            </div>
