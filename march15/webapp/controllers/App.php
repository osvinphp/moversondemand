...
class App extends CI_Controller {
	...
	public function index(){
		//unset($_SESSION['sitePassord']);

		if(isset($_POST['subPass'])){
			$password = md5($this->input->post('pasCode'));
			$var = $this->Admin_model->select_data('*','tbl_sitePassword',array('password'=>$password));
			if(!empty($var)){
				$data['sitePass'] = $var[0];
				$_SESSION['sitePassord'] = $var[0];
				//redirect(base_url('home'));
			}else{
				$this->session->set_flashdata('msg1', 'Inavlid password');
				$data['sitePass'] = '';
			}
		}
		// elseif($_SESSION['sitePassord']){
		// 		redirect(base_url('home'));
		// }
		// $this->load->view('Template/header',$data);
		//$this->load->view('starton');
		// $this->load->view('Template/footer');
		if (isset($_SESSION['sitePassord'])) {
			$this->load->view('Template/header',$data);
	...
	public function MyProfile(){
		...
		if($output->ResponseCode == 1){
			$profile['info'] = $output->Response[0];
			$_SESSION['user_details']->fname = $output->Response[0]->fname;
			$_SESSION['user_details']->lname = $output->Response[0]->lname;
	...
	public function fbLogin(){
		...
		$var = $this->Admin_model->select_data('*','tbl_users',array('fb_id'=>$data['fb_id'],'email'=>$data['email']));
	...
	public function myLogin(){
		if(isset($_POST['mydata'])){
			...
			if(isset($_SESSION['fbdata'])){
				$var = $this->Admin_model->select_data('*','tbl_users',array('fb_id'=>$_SESSION['fbdata']['fb_id']));
			}else{
				$var = $this->Admin_model->select_data('*','tbl_users',array('google_id'=>$_SESSION['gogledata']['google_id']));
			}
			$_SESSION['user_details'] = $var[0];
			redirect('book');
		}
	}
/////////////////////////////////////////////////////////////////// Booking Tab ////////////////////////////////////////////////////////////
	...
	public function page5(){		
		if(isset($_POST['datepick'])){
			...
			$srt = $this->Admin_model->convertutc($explode[0]);
			$srt1 = $this->Admin_model->convertutc($explode[1]);
			$_SESSION['calculatedSpecificFare']['booking_date'] = $_SESSION['time_slot'];
			$_SESSION['calculatedSpecificFare']['slot_starttime'] = $srt;
			$_SESSION['calculatedSpecificFare']['slot_endtime'] = $srt1;	
			$_SESSION['calculatedSpecificFare']['time_zone_start'] = $_SESSION['time_slot'].' '.$srt;
			$_SESSION['calculatedSpecificFare']['time_zone_end'] = $_SESSION['time_slot'].' '.$srt1;
        }
		// echo "<pre>";print_r($_SESSION);die;
		$this->load->view('Template/inner/header');
		$this->load->view('Template/inner/Left_sidebar');
			...
			if(isset($referdata) && !empty($referdata)){
				foreach ($referdata as $key => $value) {
					$daa .='
						<div class="promo-code-listing">
						  <div class="pcl-left-img"><img src="'.base_url('public').'/images/referral-code.png" class="img-responsive"></div>
						  <div class="pcl-right-content text_grey"><p class="main-content"><b>'.$value->referdata->referal_percentage.'% off</b> on your next referral. Avail offer max upto $'.$value->referdata->referal_amount.'.</p><p class="content-subheading">Referred by'.$value->referbyname->fname.' '.$value->referbyname->lname.'</p></div>
						</div>';
				}					
			}
		}
...
//////////////////////////////////////////////////////////////////// Your Moves ///////////////////////////////////////////////////////////
	...
	public function yourMoveListData(){
		...
		if($output->ResponseCode == 1){
			...
			foreach($myData as $key=>$value){
				...
				if($_POST['type'] == 3 && $value->move_data[0]->status == 3){
					$pcre = $value->pricing_data[0]->total_price;
				}else{
					$pcre = $value->pricing_data[0]->min_estimate_price.' - $'.$value->pricing_data[0]->max_estimate_price;
				}		
		       	// date_default_timezone_set('UTC');		       
				...
				$data .=  '
					<tr  data-toggle="collapse" data-target="#accordion'.$key.'" class="clickable clickable'.$key.'">
						...
						<td class="NUmber_Font">$'.$pcre.'<button class="move-label completed">'.$text.'</button></td>
						        	...
						        	<div class="row">
										...
										<div class="col-md-12 col-lg-5 col-sm-12 main-center-border-mdb">
											<div class="mdb-content">
												<h4 class="NUmber_Font price">$'.$pcre.'</h4><p class="time-date">MoveID #'.$value->id. ' with '.$nameS[$vehicleId].'<br>'.$date.'  '.$ss.'</p>
	...
	public function Edit_Move($id,$vehicle=null){
		...
		// $ssr = $this->Admin_model->convertutc($moveDa->slot_starttime);
		// $ssr1 = $this->Admin_model->convertutc($moveDa->slot_endtime);
		// $esx = substr($moveDa->time_zone_start,0,10);
		// $esx1 = substr($moveDa->time_zone_end,0,10);
		// print_r($esx);die;
		$edit_array = [
			'user_id'=>$_SESSION['user_details']->id,
			...
			'time_zone_end'=>$moveDa->time_zone_start,
			'book_id'=>$id,
   ...
   //  		$check_end_time = strtotime($output->response->booking_data[0]->booking_date.' '.$moveDa->slot_endtime);
	...
	public function book_select_time($idd=null){
		...
		// 	redirect('App/select_time');
		// }
		if(isset($_POST['reesubmit'])){
			 $_SESSION['retryBook']['card_id'] = $idd;
			redirect('App/bookeditdata');
		}
		...
		if(isset($_POST['timesub'])){
			...
			$_SESSION['retryBook']['slot_starttime'] = $srt;
			$_SESSION['retryBook']['slot_endtime'] =  $srt1;
			$_SESSION['retryBook']['time_zone_start'] = $_SESSION['time_slot'].' '.$srt;
			$_SESSION['retryBook']['time_zone_end'] = $_SESSION['time_slot'].' '.$srt1;
			redirect('App/bookeditdata');
		}else{
			redirect('App/select_time');
		}		
	...
	public function cardList($move_id){
		if(isset($_POST['retrysubmit'])){
			...
			$data12 = [
				...
				'card_id'=>$output->response->booking_data[0]->card_id
			];
			...
			$context  = stream_context_create($options);
			$result = file_get_contents($url, false, $context);
			$output123 = json_decode($result);
			//echo "<pre>";print_r($output);die;
			if($output->ResponseCode == 1) { /* Handle error */ 
				$responseData['cardlist'] =  $output123->Response;
				$_SESSION['bookCharge'] = $output123->setData[0]->min_booking_charge;
			}else{
				$error_msg ="NO DATA FOUND";			
			}
			$sa['bo_id'] = $output->response->booking_data[0]->card_id;
			$this->load->view('Template/inner/header',$sa);
			$this->load->view('Template/inner/Left_sidebar',$responseData);
	...
	public function timeslot(){
	    ...
	    $sg1 = $this->convertTime($current);
		$check_time = strtotime($sg1);
	...
	public function getfree(){
		$mover = $this->input->post('move');
		$vehcile = $this->input->post('vehc');
		$origin = $_POST['origin'];
	    $destination = $_POST['destination'];
		$url = "https://maps.googleapis.com/maps/api/directions/json?mode=driving&origin=".$origin."&destination=".$destination."&sensor=false&key=AIzaSyDurzeFEiJdNJvMe72sxiCIxsOh0YT7YPY";    
	    $json = file_get_contents($url);
		...
		$user = json_decode(file_get_contents('http://movers.com.au/Admin/api/User/getVehicleMove'));
		$vechile = $user->Response->vehicledata;
		...
		if($mover == 1){
			$move_charge = $vechData->movers_charges1;
		}else{
			$move_charge = $vechData->movers_charges2;
		}
		$hoursCharged = round(($mapData['duration_value'] / 60) % 60);
		$km_charges   =  round($vechData->km_charges*$mapData['distInKms'],2);
		$hour_charges = round($hoursCharged * $move_charge,2);
		$min_hour = round($vechData->min_minutes * $move_charge,2);
		$max_hour = round($vechData->max_minutes * $move_charge,2);
		$min_fare = round($km_charges + $hour_charges + $min_hour,2);
		$max_fare = round($km_charges + $hour_charges + $max_hour,2);
		echo "$".$min_fare."-"."$".$max_fare;
	}
}
